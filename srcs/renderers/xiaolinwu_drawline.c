...
/*   Updated: 2025/11/04 13:13:42 by joesanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	xiaolinwu_drawline_horizontal(t_image image, t_pixel p0, t_pixel p1)
{
	...
	{
		dist = y - (int) y;
		putpixel(image, p0.x, y, transparency(0xffffffff, dist));
		putpixel(image, p0.x++, y + 1, transparency(0xffffffff, 1 - dist));
		y += m;
	}
}
...
void	xiaolinwu_drawline_vertical(t_image image, t_pixel p0, t_pixel p1)
{
	...
	{
		dist = x - (int) x;
		putpixel(image, x, p0.y, transparency(0xffffffff, dist));
		putpixel(image, x + 1, p0.y++, transparency(0xffffffff, 1 - dist));
		x += m;
	}
}
